# iPhone 备忘录 -> Obsidian 自动同步（晚间）

## 目标
- iPhone 上继续使用「备忘录」口述记录。
- Mac 在每天 19:00、20:00、21:00、22:00、23:00 自动检查并尝试把内容并入当天 Obsidian 日记。
...
- 19:00/20:00/21:00/22:00/23:00 自动导入到当天 Obsidian 日记的 `（今日备忘）...（今日备忘结束）` 区块。
- 已默认开启：写入 Obsidian 成功后，自动清空《晨间备忘录》正文（保留标题）。
- 每次成功同步前，会先把本次口述内容归档到本地 `logs/notes-sync-archive/`，便于回溯。

## 清空备忘录也可继续用
- 你可以在同一个《晨间备忘录》里随时删除或清空内容，然后继续口述。
